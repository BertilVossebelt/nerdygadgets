...
function createTable($cart, $databaseConnection) {
    ...
    if ($totalamount == 0) {
        ...
        <center><input style='height: 48px; width: 240px; margin-top: 15px; border-radius: 10px' type='submit' name='submit' value='Bestellen'></center> </form>";
    } else {
        $table .= "<tr><th>Totaal:</th><th><!--afbeelding--></th><th>$totalamount</th><th>$total</th></table>
        <form method='get' action='RedirectiDeal.php'>
        <input type='text' name='value' value='$total' hidden>
        <center><input style='height: 48px; width: 240px' type='submit' name='submit' value='Bestellen'></center> 
        </form>";
    }
    return $table;
}
